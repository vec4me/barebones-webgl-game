...
src_dir = Path("src")
build_file = Path("build/main.html")
...
def process_text(text):
  ...
  text = re.sub(r"\bnew ([a-zA-Z0-9_]+)",
        lambda m: f"new {snake_to_pascal(m.group(1))}", text)
  text = re.sub(r"([a-z0-9])_([a-z0-9])",
  ...
  text = re.sub(r'^\s*\n', '', text,
        flags=re.MULTILINE)  # remove empty lines
  text = re.sub(r"fn\(([^{]*)\)", r"(\1) =>", text)  # anonymous functions
  text = re.sub(r"const (([a-zA-Z0-9_]+)(\.[a-zA-Z0-9_]+)+.* =>)", r"\1",
        text)  # remove the consts that we added lol
  text = re.sub(r"const ([a-zA-Z0-9_]+\[.* =>)", r"\1",
        text)  # remove the consts that we added lol
  return text
